...
export const expensesApi = emptySplitApi.injectEndpoints({
  ...
  endpoints: (build) => ({
    createExpense: build.mutation<void, { data: ExpenseData }>({
      query: ({ data }) => ({
    ...
    getAllExpenses: build.query<ExpenseResponse[], void>({
      query: () => EXPENSES_URL,
    }),
    ...
    updateExpense: build.mutation<void, { data: ExpenseResponse }>({
      query: ({ data }) => ({
        url: EXPENSES_URL, // Assuming API endpoint for updating a specific expense
